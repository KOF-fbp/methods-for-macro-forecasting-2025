...
fit_midas_safe <- function(call_args, newdata, horizon) {
  fit <- try(do.call(midasr::midas_r, call_args), silent = TRUE)
...
midas_base_call <- list(
  formula = y_train_ts ~ midasr::mls(y_train_ts, k = 1, m = 1) + midasr::fmls(x_train, k = 2, m = 3),
  start = list(x_train = rep(0, 3))
)

midas_trend_call <- midas_base_call
midas_trend_call$formula <- y_train_ts ~ trend_train + midasr::mls(y_train_ts, k = 1, m = 1) + midasr::fmls(x_train, k = 2, m = 3)
